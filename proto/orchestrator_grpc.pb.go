...
const (
	...
	OrchestratorService_StreamScreenshots_FullMethodName    = "/edgemesh.OrchestratorService/StreamScreenshots"
...
type OrchestratorServiceClient interface {
	...
	// Screenshot-based streaming (for Android / devices without WebRTC)
	StreamScreenshots(ctx context.Context, in *StreamScreenshotsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ScreenshotFrame], error)
...
func (c *orchestratorServiceClient) StreamScreenshots(ctx context.Context, in *StreamScreenshotsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ScreenshotFrame], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &OrchestratorService_ServiceDesc.Streams[0], OrchestratorService_StreamScreenshots_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[StreamScreenshotsRequest, ScreenshotFrame]{ClientStream: stream}
...
type OrchestratorService_StreamScreenshotsClient = grpc.ServerStreamingClient[ScreenshotFrame]
...
type OrchestratorServiceServer interface {
	...
	// Screenshot-based streaming (for Android / devices without WebRTC)
	StreamScreenshots(*StreamScreenshotsRequest, grpc.ServerStreamingServer[ScreenshotFrame]) error
	// File download ticket
...
func (UnimplementedOrchestratorServiceServer) StreamScreenshots(*StreamScreenshotsRequest, grpc.ServerStreamingServer[ScreenshotFrame]) error {
	return status.Error(codes.Unimplemented, "method StreamScreenshots not implemented")
}
...
func _OrchestratorService_StreamScreenshots_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StreamScreenshotsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(OrchestratorServiceServer).StreamScreenshots(m, &grpc.GenericServerStream[StreamScreenshotsRequest, ScreenshotFrame]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type OrchestratorService_StreamScreenshotsServer = grpc.ServerStreamingServer[ScreenshotFrame]
...
var OrchestratorService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "StreamScreenshots",
			Handler:       _OrchestratorService_StreamScreenshots_Handler,
			ServerStreams: true,
		},
	},
